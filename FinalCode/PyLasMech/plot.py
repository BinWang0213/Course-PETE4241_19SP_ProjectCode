...
def plotLogs(LogData,CurveNames,Depth_Range=None,NumFigCurves=2,NumCurvesPattern=[],XLims={}):
    ...
    Depth_unit=1
    if(param.Units[0]=="ft"): Depth_unit=0.3048
    elif(param.Units[0]=="m"): Depth_unit=1
    ...
    NumCols=int(NumCols)
    if(len(NumCurvesPattern)!=0):#User defined figure pattern
        NumCols=len(NumCurvesPattern)
    ...
    for col in range(NumCols):#col by col plot
        CurvesInThisCol=NumFigCurves
        if(len(NumCurvesPattern)!=0):#User defined figure pattern
            CurvesInThisCol=NumCurvesPattern[col]
        for i in range(CurvesInThisCol):
            ...
            ax=axs[col].twiny()
            ax.plot(Curve, Depth*Depth_unit, color,lw=1.5)
